...
async function onToolClick(_: ToolContext, event: ToolEvent): Promise<void>
{
    ...
    else
    {
        ...
        {
                await OBR.notification.show(Translation.t('drawing.maximumLineLengthReached'), "ERROR");
            return;
        }
...
function onToolMove(_: ToolContext, event: ToolEvent)
{
    ...
    update((line: Curve) =>
    {
        line.points[line.points.length - 1] = BSCACHE.gridSnap ? snapped : event.pointerPosition;
    });
}
