...
function onToolMove(_: ToolContext, event: ToolEvent) {
    ...
    update((mappyGon: Curve) => {
        ...
        mappyGon.points[mappyGon.points.length - 1] = BSCACHE.gridSnap ? snapped : event.pointerPosition;
    });
}

function onKeyDown(_: ToolContext, event: KeyEvent, _depth: number) {
    if (!interaction)
        return;
    if (event.key == "Escape") {
        cancelDrawing();
...
async function enterEditMode(context: ToolContext) {
    ...
    if (visionOn) {
        ...
        await OBR.tool.setMetadata(`${Constants.EXTENSIONID}/vision-tool`,
            {
                [`${Constants.EXTENSIONID}/elevationEditor`]: true
            });
        const width = await OBR.viewport.getWidth();
...
export const elevationMode = { onToolClick, onToolMove, onKeyDown, enterEditMode, cancelDrawing, Deactivate };