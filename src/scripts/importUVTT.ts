import OBR, { Image, buildCurve, buildImageUpload, buildSceneUpload, Curve, Vector2 } from "@owlbear-rodeo/sdk";
...
export async function FogImportEntry(importType: string, importData: any, importDpi: number, importMapId: string) 
{
    ...
    {
        return await OBR.notification.show(Translation.t('import.notifications.invalidDpi'), "ERROR");
    }
    ...
    if (importMapId !== "")
    {
        ...
        {
            return await OBR.notification.show(Translation.t('import.notifications.mapNotFound'), "ERROR");
        }
    } else
    {
        return await OBR.notification.show(Translation.t('import.notifications.noMapSelected'), "ERROR");
    }
...
export async function ImportScene(importData: UVTT, _errorElement: HTMLDivElement)
{
    ...
    try 
    {
        ...
        const scene = buildSceneUpload()
            .baseMap(image)
            .fogFilled(true)
            .name(Translation.t('import.notifications.newUvttSceneName'))
            .items(importedObjects)
            .build();
        await ProgressTheBar(80);
        await OBR.assets.uploadScenes([scene], true);
        await ProgressTheBar(100, Translation.t('import.notifications.sceneUploadComplete'));
    }
    catch (error: any)
    {
        await OBR.notification.show(Translation.t('import.notifications.errorPrefix', { message: error.message }), "ERROR");
    }
}
...
async function BatchUpload(dataObjects: any[]): Promise<string>
{
    ...
    return await OBR.notification.show(Translation.t('import.notifications.finishedImporting', { count: dataObjects.length }), "SUCCESS");
}
...
async function ProgressTheBar(int: number, message = Translation.t('import.notifications.done')): Promise<void>
{
