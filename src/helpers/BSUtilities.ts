...
export async function HardwareWarning(self: boolean, party?: Player[])
{
    ...
    if (showWarning)
    {
        if (self)
        {
            await OBR.notification.show(Translation.t('warnings.hardwareAccelerationSelf'), "WARNING");
        ...
        else if (party)
        {
            ...
            if (warnedPlayers.length > 0) await OBR.notification.show(Translation.t('warnings.hardwareAccelerationParty', { players: warnedPlayers.map(x => x.name).join(", ") }), "WARNING");
        }
    }
}
...
export function adjustPoints(points: Vector2[], adjustment: Vector2): Vector2[] {
