...
const isTokenLayer = (layer: Item['layer']): boolean => {
  return layer === 'CHARACTER' || layer === 'MOUNT' || layer === 'ATTACHMENT' || layer === 'PROP';
};
...
function isTokenWithVision(item: Item) {
  return isTokenLayer(item.layer) && item.metadata[`${Constants.EXTENSIONID}/hasVision`];
}
...
function isTokenWithVisionIOwn(item: Item) {
  return isTokenLayer(item.layer)
    && item.createdUserId === BSCACHE.playerId
...
export {
  ...
  isLocalSpectre,
  isVisionLine,
  isLocalPersistentLight,
  isVisionLineAndEnabled,
  isLocalVisionWall,
  isLocalVisionLight,
  isBrushSquare,
  isActiveVisionLine,
  isTokenWithVision,
  isIndicatorRing,
  isTokenWithVisionIOwn,
  isTokenWithVisionForUI,
  isTorch,
  isAutohide,
};
